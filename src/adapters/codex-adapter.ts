...
export class CodexAdapter implements AgentAdapter {
  ...
  async execute(intent: AgentIntent): Promise<AgentResult> {
    ...
    const result = await invokeConnector('codex', ['exec', intent.task], intent)
    if (!context) {
      return result
    }
    ...
    child.once('error', (error) => {
      ...
      settle({
        error: {
          ...
          message: `Codex CLI 调用失败: ${error.message}`,
        },
    ...
    child.once('close', (exitCode, signal) => {
      if (didTimeout) {
        settle({
          error: {
            ...
            message: `Codex CLI 执行超时 (${timeoutMs}ms)`,
          },
          intentId: intent.intentId,
          output: '',
          success: false,
      ...
      settle({
        error: {
          ...
          message: `Codex CLI 返回非零退出码: ${exitCode ?? 'null'}`,
        },
        intentId: intent.intentId,
        output: stdout.trim(),
        success: false,
